...
export default function PatternSelector({ selected, onChange, disabled }: PatternSelectorProps) {
  return (
    ...
    <div className="grid grid-cols-2 gap-1.5 w-full max-w-[300px] mx-auto mb-3 md:flex md:flex-wrap md:justify-center md:max-w-none md:gap-3 md:mb-14">
      ...
      {FREE_PATTERNS.map((p) => {
        ...
        return (
          <button
            ...
            disabled={disabled}
            className={[
              "w-full md:w-auto flex flex-col items-center gap-0.5 px-2 py-1.5 md:px-7 md:py-3 rounded-full border transition-all duration-300",
          ...
          >
            <span className="[font-family:var(--font-jost)] text-[11px] md:text-[12px] font-[400] tracking-[-0.01em] leading-none">
              {p.name}
            </span>
            <span className="[font-family:var(--font-jost)] text-[9px] md:text-[9px] tracking-[0.14em] uppercase opacity-55 leading-none">
              {p.timing}
            </span>
          </button>
        );
      ...
      {LOCKED_PATTERNS.map((p) => (
        <a
          key={p.name}
          href="/pricing"
          className="w-full md:w-auto flex flex-col items-center gap-0.5 px-2 py-1.5 md:px-7 md:py-3 rounded-full border border-[rgba(255,255,255,0.18)] bg-transparent text-[rgba(255,255,255,0.60)] opacity-60 hover:opacity-80 hover:text-[rgba(255,255,255,0.85)] hover:border-[rgba(255,255,255,0.28)] transition-all duration-200"
        >
          <span
            className="[font-family:var(--font-jost)] text-[11px] md:text-[12px] tracking-[-0.01em] leading-none"
